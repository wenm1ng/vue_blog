 ...
 * @Time: 14:07
 */
layui.define(['admin','jquery', 'table'], function(exports) {
	"use strict";
	...
	yutons_sug.prototype.render = function(opt) {
		...
		opt.catetype = opt.catetype || 0;
		saveId = opt.saveId

		//初始化输入框提示容器
		if($("#sugItem_" + saveId).length == 0){
			$("#" + opt.id).after("<div id='sugItem_"+saveId+"' style='background-color: #fff;display: none;z-index:1;position: absolute;width:100%;'></div>");
		}
	...
	function getSug(opt) {
		...
		admin.req({
			type: "get",
			url: opt.urlBak+kw+'&type='+catetype,
			success: function(data) {
				...
				layui.each(data.data, (index, item) => {
					...
					//if (item[sessionStorage.getItem("inputId")].indexOf(decodeURI(kw)) >= 0) {
						html +=
							"<div id = " +item['id']+ " class='item' style='padding: 3px 10px;cursor: pointer;' onmouseenter='getFocus(this)' onClick='getCon(this," +item['id']+ ");'>" +
	...
	window.getCon = function(obj,title_id) {
		let value = $(obj).text();
